...
#     steps.pay_with_card(INVALID_CVC_CARD)
#
#     # 3. Проверяем результат (логика проверки остается здесь)
#     error_text = payment_page.get_error_message_text()
#     assert "Неверный формат" in error_text, f"Ожидалась ошибка 'Неверный формат', но получила: {error_text}"
#
#
...
#     """Тест 4: Оплата с просроченной картой."""
#
#     # 1. Создаем экземпляр шагов
#     steps = PaymentSteps(payment_page)
#
#     # 2. Вызываем готовый метод
#     steps.pay_with_card(EXPIRED_CARD)
#
...
#
# def test_invalid_cvc_format(payment_page):
#     card = INVALID_CVC_CARD
#     payment_page.fill_card_number(card["number"])
#     payment_page.fill_holder(card["holder"])
#     payment_page.fill_cvc(card["cvc"])
#     payment_page.fill_date(card["month"], card["year"])
#
#     # Вместо прямого поиска элемента вызываем метод из PaymentPage
#     payment_page.submit()
#
#     # Получаем текст ошибки и делаем проверку
#     error_text = payment_page.get_error_message_text()
#     assert "Неверный формат" in error_text, f"Ожидалась ошибка по формату, но получила: {error_text}"
#
#
# def test_empty_expiry_year(payment_page):
#     card = EXPIRED_CARD
...
#     payment_page.submit()
#
#     # Аналогично для второго теста
#     error_text = payment_page.get_error_message_text()
#     assert "Истёк срок" in error_text, f"Ожидалась ошибка по сроку действия, но получила: {error_text}"
#
# from autotests_python.data.test_data import EXPIRED_CARD, INVALID_CVC_CARD


# def test_empty_expiry_year(payment_page):
#     # 2. Используем данные из констант
#     card_data = EXPIRED_CARD
#
#     payment_page.fill_card_number(card_data["number"])
#     payment_page.fill_holder(card_data["holder"])
#     payment_page.fill_cvc(card_data["cvc"])
#     payment_page.fill_date(card_data["month"], card_data["year"])
#
#     payment_page.submit()
#
...
#     assert err.is_displayed(), "Ошибка 'Неверный формат' не отображается"
    # ... ожидание ошибки и assert остаются без изменений
# def test_empty_expiry_year(payment_page):
#     #Тест 4: Оплата с просроченной картой
#     payment_page.fill_card_number("4444 4444 4444 4441")
